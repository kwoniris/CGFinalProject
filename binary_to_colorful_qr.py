...
def binary_to_color_image(binary_file, output_folder, image_name):
    """
    Converts binary data to a color image based on 3-byte chunks.
    ...
    # rgb_colors = [
    #     (chunk[0], chunk[1], chunk[2]) if len(chunk) == 3 else (chunk[0], chunk[1], 0)
    #     for chunk in chunks
    # ]
    rgb_colors = [
    ...
    image_size = math.ceil(math.sqrt(num_pixels))

    # Pad with black (0, 0, 0) to fit the square
    padded_colors = rgb_colors + [(0, 0, 0)] * (image_size**2 - num_pixels)

    # Create the image array
    image_array = np.array(padded_colors, dtype=np.uint8).reshape((image_size, image_size, 3))
...
def process_all_binary_files(input_folder, output_folder):
    """
    Processes all binary files in the input folder and generates color images.
    ...
    for binary_file in binary_files:
        ...
        binary_to_color_image(binary_file_path, output_folder, image_name)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Convert binary .bin files to color images based on 3-byte chunks."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--identifier", required=True, help="Identifier for naming output images."
    )
